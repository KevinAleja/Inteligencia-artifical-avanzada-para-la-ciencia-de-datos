...
# 5. División de datos para entrenamiento, prueba y validación
X_temp, X_test, y_temp, y_test = train_test_split(
    X_scaled, y_original, test_size=0.2, random_state=42
)

X_train, X_val, y_train, y_val = train_test_split(
    X_temp, y_temp, test_size=0.25, random_state=42  # 0.25 * 0.8 = 0.2 (20% de validación)
)
...
print(f"Validación    - MSE: {mse_val:.6f}, R^2: {r2_val:.4f}")
print(f"Test          - MSE: {mse_test:.6f}, R^2: {r2_test:.4f}")
...
plt.title(f'Validación: R² = {r2_val:.3f}')
plt.xlabel('Valores Reales')
plt.ylabel('Predicciones (MW)')
plt.grid(True)
